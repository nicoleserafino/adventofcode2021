...
def solve(scanners):
    ...
    while len(positions) < len(scanners):
        for i, j, v in match(scanners, hashes):
            ...
            beacons |= new_beacons
    return len(beacons), max(np.abs(x - y).sum() for x, y in combinations([positions[i] for i in range(len(scanners))], 2))

def Main():
    data = [x.split("---\n")[-1].split("\n") for x in open("2021/day19/input.txt").read().split("\n\n")]
    scanners = [np.array([list(map(int, y.split(","))) for y in x]) for x in data] 
    
    p1, p2 = solve(scanners)
